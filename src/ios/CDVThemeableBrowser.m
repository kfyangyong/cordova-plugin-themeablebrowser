...
- (void)close:(CDVInvokedUrlCommand*)command
{
    if (self.themeableBrowserViewController == nil) {
        NSLog(@"themable.close() called but it was already closed.");
        return;
    }
...
- (void)openInThemeableBrowser:(NSURL*)url withOptions:(NSString*)options{
    ...
    if (browserOptions.cleardata) {
        ...
        [dataStore removeDataOfTypes:[WKWebsiteDataStore allWebsiteDataTypes] modifiedSince:dateFrom completionHandler:^{
            ...
            self.themeableBrowserViewController.webView.configuration.processPool = [[WKProcessPool alloc] init]; // create new process pool to flush all data
        }];
    }
...
- (void)show:(CDVInvokedUrlCommand*)command withNoAnimate:(BOOL)noAnimate
{
    ...
    __block CDVThemeableBrowserNavigationController* nav = [[CDVThemeableBrowserNavigationController alloc] initWithRootViewController:self.themeableBrowserViewController];
...
- (void) rePositionViews {
    ...
    if (@available(iOS 11.0, *)) {
        additionalOffset = self.parentViewController.view.safeAreaInsets.top;
    }
    CGFloat webviewOffset = _browserOptions.fullscreen ? 0.0 : toolbarHeight + additionalOffset;
